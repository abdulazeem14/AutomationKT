		...
		if(catergory.getText().equals(Category))
		{
			...
			for(WebElement plppage:pom.getHomePage().getPLP())
			{
				if(plppage.getText().equals(td.get("Product"+i+" Name")))
				{
					...
					log.add(td.get("Product"+i+" Name")+" Selected in PLP is Verified with ProductName in PDP"+pom.getPDP().pdpName().getText()); 
...
@Test(dataProvider="category")
public void categoryIn(HashMap<String,String> td) throws IOException, InterruptedException {
log=new ArrayList<String>();
String Category=td.get("Category");
	boolean flag=false;
	int i=1;
	...
	if(catergory.getText().equals(Category)) {
		...
		for(WebElement plppage:pom.getHomePage().getPLP())
		{
			if(plppage.getText().equals(td.get("Product"+i+" Name")))
			{
				...
				log.add(td.get("Product"+i+" Name")+" Selected in PLP is Verified with ProductName in PDP"+pom.getPDP().getProductName()); 
